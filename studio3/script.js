(function(){
  ...
  const granimInstance = new Granim({
    ...
    states: {
      "default-state": {
        gradients: [
          ['#00b4d8', '#90e0ef', '#0077b6'],
          ['#48cae4', '#ade8f4', '#00b4d8'],
          ['#0096c7', '#caf0f8', '#48cae4'],
          ['#00b4d8', '#90e0ef', '#023e8a'],
        ],
  ...
  const seaweedPositions = [60, 150, 280, 420, 600, 750, 900, 1050, 1130];
    ...
    class Fish {
      ...
      move(){
        ...
        if (d < this.fleeRadius){
          ...
          const targetSpeedX = p.cos(angle) * this.fleeSpeed; // ✅ fixed
          const targetSpeedY = p.sin(angle) * this.fleeSpeed; // ✅ was missing
        ...
        if (p.random() < 0.005){
          bubbles.push(new Bubble(this.x, this.y - this.size * 0.3)); // ✅ fixed capital B
        }
      }
      ...
      draw(){
        ...
        p.fill(20);
        p.circle(this.size * 0.25, -this.size * 0.06, this.size * 0.09); // ✅ fixed eye position
